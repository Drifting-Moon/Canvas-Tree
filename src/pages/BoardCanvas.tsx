...
export default function BoardCanvas() {
    ...
    const [notes, setNotes] = useState<NoteType[]>(() => {
        const saved = localStorage.getItem(`board_notes_${id}`);
        return saved ? JSON.parse(saved) : [];
    });

    const [count, setCount] = useState<number>(() => {
        const cnt = localStorage.getItem(`board_count_${id}`);
    ...
    useEffect(() => {
        if (!id) return;
        // Move to board-specific unique keys so they don't overwrite each other
        localStorage.setItem(`board_notes_${id}`, JSON.stringify(notes));
        localStorage.setItem(`board_count_${id}`, JSON.stringify(count));
    }, [notes, count, id]);
        ...
        >
            ...
            >
                ...
                <button className="clear-btn" onClick={(e) => {
                    ...
                    setNotes([]); 
                    setCount(0); 
                    localStorage.removeItem(`board_notes_${id}`);
                    localStorage.removeItem(`board_count_${id}`);
                }}>
                    {count}
                </button>
            </h1>
